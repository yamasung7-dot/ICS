...
(function () {
	...
	function getPreviews() {
		return (typeof Preview !== 'undefined' && Array.isArray(Preview.all)) ? Preview.all : [];
	}

	function createSurface() {
		if (surface || typeof THREE === 'undefined' || !Canvas?.scene) return surface;
		const geometry = new THREE.PlaneGeometry(6, 6);
		const material = new THREE.MeshBasicMaterial({ transparent: true, depthWrite: false, depthTest: false, color: 0xffffff });
	...
	function installPreview(preview) {
		...
		console.info('[ICS] DODFP bridge installed:', { version: VERSION, preview, renderer, renderTarget: target, texture: target.texture });
		return true;
	}
	...
	function install() {
		createSurface();
		let count = 0;
		getPreviews().forEach(preview => { if (installPreview(preview)) count++; });
		installed = true;
	...
	function returnToMoveTool() {
		if (typeof BarItems !== 'undefined' && BarItems.move_tool && typeof BarItems.move_tool.select === 'function') BarItems.move_tool.select();
	}

	function openCreator() {
		new Dialog({
			id: 'ics_creator', title: 'ICS — Cursed Spirit Creator', width: 430, darken: true,
			form: {
				identity: { type: 'info', text: '**IMMORTAL CURSED SPIRIT**\n\nDODFP bridge v' + VERSION + ': Blockbench geometry is rendered to a 2D texture, then that texture is fed back onto a 3D surface.' },
				state: { type: 'info', text: `**Version:** ${VERSION}\n**Probe:** ${installed ? 'CONNECTED' : 'DORMANT'}\n**Surface:** ${surfaceEnabled ? 'VISIBLE' : 'HIDDEN'}` },
				enable: { type: 'checkbox', label: 'Enable DODFP probe', description: 'Creates the offscreen 2D render texture while preserving the normal Blockbench renderer.', value: installed },
				surface: { type: 'checkbox', label: 'Show 2D → 3D surface', description: 'Displays a real 3D plane carrying the offscreen texture in the preview camera.', value: surfaceEnabled },
				buffer: { type: 'info', text: `**2D buffer:** ${TARGET_SIZE} × ${TARGET_SIZE} RGBA · depth enabled\n**Return path:** texture → THREE.Mesh plane → render` },
				warning: { type: 'info', text: '*Experimental specimen. Small buffers and a simple surface are intentional for Android/mobile stability.*' }
			},
	...
	probeTool = new Tool('ics_dodfp_probe', {
		name: 'ICS: DODFP Probe', icon: 'memory', category: 'tools', toolbar: 'main_tools', transformerMode: 'hidden',
		modes: ['edit', 'paint', 'display', 'animate', 'pose'],
		onSelect() {
			if (installed) { uninstall(); Blockbench.showQuickMessage('ICS v' + VERSION + ' DODFP probe removed'); }
			else install();
			returnToMoveTool();
		}
	});
	...
	creatorTool = new Tool('ics_cursed_spirit_creator', {
		name: 'ICS: Cursed Spirit Creator', icon: 'auto_fix_high', category: 'tools', toolbar: 'main_tools', transformerMode: 'hidden',
		modes: ['edit', 'paint', 'display', 'animate', 'pose'],
		onSelect() { openCreator(); returnToMoveTool(); }
	});

	Plugin.register('ics', {
		title: 'ICS — Immortal Cursed Spirit', author: 'TFO', description: 'Experimental DODFP rendering bridge for Blockbench.',
		about: 'Experimental 3D → 2D → 3D rendering bridge using Blockbench\'s real WebGLRenderer and a Three.js texture surface.',
		version: VERSION, icon: 'memory', variant: 'both', min_version: '4.0.0',
		onload() { Blockbench.showQuickMessage('ICS v' + VERSION + ' imported successfully'); },
		oninstall() { console.info('[ICS] Version ' + VERSION + ' imported successfully.'); },
		onuninstall() { this.onunload(); },
		onunload() {
			uninstall();
